...
`cluster` 모듈은 멤버십, 리더 선출, 분산 락, 할당 서비스를 제공해 다른 모듈(workflow, source/handlers/pipeline/sink)에서 재사용한다. 대상: Java 21, Spring Boot 3.3, Reactor 3.6.
...
- Kafka 모드
  ...
  - 애플리케이션 활성/대기 모델: 모든 애플리케이션이 기본적으로 active로 처리하면서 동시에 다른 애플리케이션의 standby 역할을 겸한다. 예를 들어 파티션이 3개면 3개 애플리케이션이 각 파티션을 active로 담당하고, 장애·종료 시 컨슈머 리밸런스 후 다른 애플리케이션이 해당 파티션을 assigned 받아 즉시 승계한다.
...
- `AssignmentService`:
  ...
  - Kafka 모드(키 할당): 파티션에 들어오는 새로운 키에 대해 자동 할당을 생성하고, 수동/수정 요청을 반영해 키→애플리케이션 매핑을 유지.
...
4) 할당:
   - Kafka 모드: Kafka 리밸런스 콜백 기반으로 토폴로지 변경을 발행하고, 파티션 assigned 시 키/설비/워크플로 핸드오프를 로드해 캐시를 준비.
